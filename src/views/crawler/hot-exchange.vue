<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" min-width="12%" label="当前价格">
        <template slot-scope="scope">
          <div v-html="scope.row.currentPriceHtml"></div>
        </template>
      </el-table-column>

      <el-table-column align="center" min-width="12%" label="波动">
        <template slot-scope="scope">
          <div v-html="scope.row.changeHtml"></div>
        </template>
      </el-table-column>

      <el-table-column align="center" min-width="12%" label="今开">
        <template slot-scope="scope">
          <div v-html="scope.row.todayPriceHtml"></div>
        </template>
      </el-table-column>

      <el-table-column align="center" min-width="12%" label="昨收">
        <template slot-scope="scope">
          <div v-html="scope.row.yesterdayPriceHtml"></div>
        </template>
      </el-table-column>

      <el-table-column align="center" min-width="12%" label="最高">
        <template slot-scope="scope">
          <div v-html="scope.row.highestPriceHtml"></div>
        </template>
      </el-table-column>

      <el-table-column align="center" min-width="14%" label="最低">
        <template slot-scope="scope">
...
<script>
  import { fetchForwardExchangeCurrencyList, fetchHotExchangeList } from '@/api/crawler'
